...
namespace RRD.FSG.RP.Web.UI.Hosted
{
    ...
    public class HostedController : BaseController
    {
        ...
        public ActionResult TaxonomyAssociationLinks(string customer, string site, bool? isProofing = false)
        {
            ...
            int taxonomyOrderFeatureMode = hostedClientPageScenarios.GetSiteFeatureModeFromCache(customer, site, "FundOrder");
            FeatureEnums.FundOrder taxonomyOrderfeature = FeatureHelper.GetFeatureModesFundOrder(taxonomyOrderFeatureMode).FirstOrDefault();
            int fundNameFormatFeatureMode = hostedClientPageScenarios.GetSiteFeatureModeFromCache(customer, site, "FundNameFormat");
            FeatureEnums.FundNameFormat fundNameFormatFeature = FeatureHelper.GetFeatureModesFundNameFormat(fundNameFormatFeatureMode).FirstOrDefault();

            viewModel.TaxonomyAssociationLinkModelData = hostedClientPageScenarios.GetTaxonomyAssociationLinks(customer, site, isProofing.Value, taxonomyOrderfeature, fundNameFormatFeature);
        ...
        public ActionResult TaxonomyAssociationHierarchyDocuments(string customer, string site, string externalID, bool? isInternalTAID = false, bool? isProofing = false)
        {
            ...
            {
                objTaxonomyAssociationHierarchyModel = hostedClientPageScenarios.GetTaxonomyAssociationHierarchyDocuments(customer, site, isProofing.Value, null, int.Parse(externalID), taxonomyOrderfeature, fundNameFormatFeature);
            }
            else
            {
                objTaxonomyAssociationHierarchyModel = hostedClientPageScenarios.GetTaxonomyAssociationHierarchyDocuments(customer, site, isProofing.Value, externalID, null, taxonomyOrderfeature, fundNameFormatFeature);
            }
        ...
        public ActionResult TaxonomySpecificDocumentFrame(string customer, string site, string externalID1, string externalID2, bool? isInternalTAID = false, bool? isProofing = false, bool? spv = false)
        {
            ...
            if (isInternalTAID.Value)
            {
                viewModel.TaxonomyAssociationData = hostedClientPageScenarios.GetTaxonomySpecificDocuments(customer, site, isProofing.Value, null, int.Parse(externalID1), fundNameFormatFeature);
            }
            else
            {
                viewModel.TaxonomyAssociationData = hostedClientPageScenarios.GetTaxonomySpecificDocuments(customer, site, isProofing.Value, externalID1, null, fundNameFormatFeature);
            }
        ...
        public ActionResult TaxonomyAssociationClientDocumentFrame(string customer, string site, string externalID1, string externalID2, bool? isITAID = false, bool? isProofing = false, bool SPV = false)
        {
            ...
            if (isITAID.Value)
            {
                viewModel.TaxonomyAssociationData = hostedClientPageScenarios.GetTaxonomyAssociationClientDocuments(customer, isProofing.Value, null, externalID2, int.Parse(externalID1));
            }
            else
            {
                viewModel.TaxonomyAssociationData = hostedClientPageScenarios.GetTaxonomyAssociationClientDocuments(customer, isProofing.Value, externalID1, externalID2, null);
            }
        ...
        public ActionResult TaxonomyAssociationDocuments(string customer, string site, bool? isProofing = false)
        {
            ...
            viewModel.TaxonomyAssociationDocumentsModel = hostedClientPageScenarios.GetTaxonomyAssociationDocuments(customer, site, isProofing.Value, taxonomyOrderfeature);
        ...
        public ActionResult XBRL(string customer, string site, string externalID, bool? isInternalTAID = false, bool? isProofing = false, bool isPID = false, Guid? requestBatchId = null)
        {
            ...
            if (isInternalTAID.Value)
            {
                viewModel.XBRLData = hostedClientPageScenarios.GetXBRLByTaxonomyAssociationIDOrExternalID(customer, isProofing.Value, int.Parse(externalID), null, isPID);
            }
            else
            {
                viewModel.XBRLData = hostedClientPageScenarios.GetXBRLByTaxonomyAssociationIDOrExternalID(customer, isProofing.Value, null, externalID, isPID);
            }
        ...
        public ActionResult RequestMaterial(string customer, string site, string externalID, bool? isInternalTAID = false, bool? isProofing = false, Guid? requestBatchId = null)
        {
            ...
            if (isInternalTAID.Value)
            {
                viewModel.TaxonomyAssociationData = hostedClientPageScenarios.GetTaxonomySpecificDocuments(customer, site, isProofing.Value, null, int.Parse(externalID), fundNameFormatFeature);
            }
            else
            {
                viewModel.TaxonomyAssociationData = hostedClientPageScenarios.GetTaxonomySpecificDocuments(customer, site, isProofing.Value, externalID, null, fundNameFormatFeature);
            }
        ...
        public ActionResult TaxonomyAssociationGroupDocuments(string customer, string site, bool? isProofing = false, int? Id = null)
        {
            ...
            viewModel.TaxonomyAssociationGroupModelData = hostedClientPageScenarios.GetTaxonomyAssociationGroupDocuments(customer, string.IsNullOrEmpty(site) ? null : site, isProofing.Value, Id == 0 ? null : Id, taxonomyOrderfeature);
