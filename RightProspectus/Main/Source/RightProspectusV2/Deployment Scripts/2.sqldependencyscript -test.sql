...
IF NOT EXISTS(SELECT principal_id FROM sys.database_principals WHERE name = 'FINCOEC\Test-pool-RPConnect') 
BEGIN
 CREATE USER [FINCOEC\Test-pool-RPConnect] FOR LOGIN [FINCOEC\Test-pool-RPConnect] WITH DEFAULT_SCHEMA = [Test-pool-RPConnect]
END

IF NOT EXISTS (SELECT  schema_name FROM   information_schema.schemata WHERE   schema_name = 'Test-pool-RPConnect' ) 
BEGIN
  EXEC sp_executesql N'CREATE SCHEMA [Test-pool-RPConnect] AUTHORIZATION [FINCOEC\Test-pool-RPConnect]' 
END
...
GRANT SELECT to rpv2sql_dependency 
    ...
    EXEC sp_addrolemember 'rpv2sql_dependency', [FINCOEC\Test-pool-RPConnect]


	EXEC sp_addrolemember 'db_datareader', [FINCOEC\Test-pool-RPConnect];
	
	EXEC sp_addrolemember 'db_datawriter', [FINCOEC\Test-pool-RPConnect]; 
	
	GRANT EXECUTE ON SCHEMA::dbo TO [FINCOEC\Test-pool-RPConnect];
