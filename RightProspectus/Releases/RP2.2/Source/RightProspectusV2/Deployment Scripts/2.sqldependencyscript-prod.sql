...
IF NOT EXISTS(SELECT principal_id FROM sys.database_principals WHERE name = 'FINCOAD\RPIngestorProdServ') 
BEGIN
	CREATE USER [FINCOAD\RPIngestorProdServ] FOR LOGIN [FINCOAD\RPIngestorProdServ] 
...
BEGIN
  EXEC sp_executesql N'CREATE SCHEMA [RPIngestorProdServ] AUTHORIZATION [FINCOAD\RPIngestorProdServ]' 
END
...
IF NOT EXISTS(SELECT principal_id FROM sys.database_principals WHERE name = 'FINCOEC\prod-pool-rpconnect') 
BEGIN
 CREATE USER [FINCOEC\prod-pool-rpconnect] FOR LOGIN [FINCOEC\prod-pool-rpconnect] WITH DEFAULT_SCHEMA = [prod-pool-rpconnect]
END

IF NOT EXISTS (SELECT  schema_name FROM   information_schema.schemata WHERE   schema_name = 'prod-pool-rpconnect' ) 
BEGIN
  EXEC sp_executesql N'CREATE SCHEMA [prod-pool-rpconnect] AUTHORIZATION [FINCOEC\prod-pool-rpconnect]' 
END
...
GRANT SELECT to rpv2sql_dependency 
    ...
    EXEC sp_addrolemember 'rpv2sql_dependency', [FINCOAD\RPIngestorProdServ]


	EXEC sp_addrolemember 'db_datareader', [FINCOAD\RPIngestorProdServ];
	
	EXEC sp_addrolemember 'db_datawriter', [FINCOAD\RPIngestorProdServ]; 	
	
	GRANT EXECUTE ON SCHEMA::dbo TO [FINCOAD\RPIngestorProdServ];
    ...
    EXEC sp_addrolemember 'rpv2sql_dependency', [FINCOEC\prod-pool-rpconnect]


	EXEC sp_addrolemember 'db_datareader', [FINCOEC\prod-pool-rpconnect];
	
	EXEC sp_addrolemember 'db_datawriter', [FINCOEC\prod-pool-rpconnect]; 
	
	GRANT EXECUTE ON SCHEMA::dbo TO [FINCOEC\prod-pool-rpconnect];
