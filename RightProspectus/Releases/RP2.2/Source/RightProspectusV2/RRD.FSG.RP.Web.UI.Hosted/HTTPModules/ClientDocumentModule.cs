...
{
    public class ClientDocumentModule : IHttpModule
    {
        /// <summary>
        ...
        /// <summary>
        /// Constructor for ClientDocumentModule
        /// </summary>
        public ClientDocumentModule()
        {
        ...
        /// Example: http://localhost:54346/ClientDocument/1?client=forethought
        /// </summary>
        ...
        void context_BeginRequest(object sender, EventArgs e)
        {
            ...
            if (context.Request.Path.ToUpper().Contains("CLIENTDOCUMENT"))
            {
                int clientDocumentId = int.Parse(Path.GetFileName(context.Request.Path));
                string clientName = context.Request.QueryString["client"];

                var clientDocumentDetails = hostedClientPageScenarios.GetClientDocumentDetails(clientName, clientDocumentId);
                if (clientDocumentDetails != null)
                {
                    context.Response.Clear();
                    context.Response.ContentType = clientDocumentDetails.MimeType;

                    DateTime lastModifiedDate = clientDocumentDetails.LastModified.Value;
                    ...
                    context.Response.BinaryWrite(clientDocumentDetails.FileData);
                    context.CompleteRequest();
                }
            }
        }
