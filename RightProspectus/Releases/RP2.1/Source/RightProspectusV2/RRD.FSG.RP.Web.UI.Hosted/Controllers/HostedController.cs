...
namespace RRD.FSG.RP.Web.UI.Hosted
{
    ...
    public class HostedController : BaseController
    {
        ...
        public ActionResult TaxonomySpecificDocumentFrame(string customer, string site, string externalID1, string externalID2, bool? isInternalTAID = false, bool? isProofing = false, bool? spv = false)
        {
            ...
            {
                viewModel.BrowserAlertText = cachedSiteText.Find(p => p.ResourceKey == "BrowserAlertText").Text;
            }
        ...
        public ActionResult TaxonomyAssociationGroupDocuments(string customer, string site, bool? isProofing = false, int? Id = null)
        {
            ...
            {
                viewModel.DocumentNotAvailableText = cachedPageText.Find(p => p.ResourceKey == "TAGD_DocumentNotAvailableText").Text;
            }
            if (cachedPageText.Exists(p => p.ResourceKey == "TAGD_ClientCustomHeader"))
            {
                viewModel.ClientCustomHeader = cachedPageText.Find(p => p.ResourceKey == "TAGD_ClientCustomHeader").Text;
            }

            if (cachedPageText.Exists(p => p.ResourceKey == "TAGD_FootnotesHeaderText"))
            {
                viewModel.FootnotesHeaderText = cachedPageText.Find(p => p.ResourceKey == "TAGD_FootnotesHeaderText").Text;
            }
            if (cachedPageText.Exists(p => p.ResourceKey == "TAGD_Glossary"))
            {
                viewModel.Glossary = cachedPageText.Find(p => p.ResourceKey == "TAGD_Glossary").Text;
            }
            if (cachedPageText.Exists(p => p.ResourceKey == "TAGD_UnderlayingFundGridFundNameColumnText"))
            {
                viewModel.UnderlayingFundGridFundNameColumnText = cachedPageText.Find(p => p.ResourceKey == "TAGD_UnderlayingFundGridFundNameColumnText").Text;
            }
            ...
            int groupSearchMode = hostedClientPageScenarios.GetPageFeatureModeFromCache(customer, site, viewModel.PageId, "AllCategories");
            List<FeatureEnums.AllCategories> featureModeGroupSearch = FeatureHelper.GetFeatureModesAllCategories(groupSearchMode);

            if (featureModeGroupSearch.Contains(FeatureEnums.AllCategories.Enabled))
            {
                viewModel.EnableGroupSearch = true;
            }
        ...
        public PartialViewResult TaxonomyAssoicationGroupTaxonomyAssoicationGrid(string customer, string site, bool? isProofing = false, int? Id = null)
        {
            TaxonomyAssociationGroupViewModel viewModel = new TaxonomyAssociationGroupViewModel();
            bool getProductionVersion = !isProofing.Value;
            List<HostedPageText> cachedPageText = hostedClientPageScenarios.GetPageTextFromCache(customer, site, 7).FindAll(p => p.IsCurrentProductionVersion == getProductionVersion);
