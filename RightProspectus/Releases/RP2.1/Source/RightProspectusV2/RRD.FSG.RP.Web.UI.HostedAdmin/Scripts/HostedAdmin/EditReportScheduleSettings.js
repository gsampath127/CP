...
function BindFTP() {
    ...
    $("#ComboTransferType").change(function () {
        ...
        } else if (Number($("#ComboTransferType").val()) == 1 || Number($("#ComboTransferType").val()) == 2) {
            $("#txtFTPServerIP").val('');
            $("#txtFTPUsername").val('');
            $("#txtFTPPassword").val('');

            if (Number($("#ComboTransferType").val()) == 1) {
                $("#lblTransServerIp").text("SFTPServerIP");
                $("#txtFTPServerIP").attr("placeholder", "SFTPServerIP");

                $("#lblTransUserName").text("SFTPUserName");
                $("#txtFTPUsername").attr("placeholder", "SFTPUserName");

                $("#lblTransPassword").text("SFTPPassword");
                $("#txtFTPPassword").attr("placeholder", "SFTPPassword");

            }
            else if (Number($("#ComboTransferType").val()) == 2) {
                $("#lblTransServerIp").text("FTPServerIP");
                $("#txtFTPServerIP").attr("placeholder", "FTPServerIP");

                $("#lblTransUserName").text("FTPUserName");
                $("#txtFTPUsername").attr("placeholder", "FTPUserName");

                $("#lblTransPassword").text("FTPPassword");
                $("#txtFTPPassword").attr("placeholder", "FTPPassword");
            }
...
function ValidateSave() {
    ...
    if ($("#ComboTransferType").val() == "-1") {
        ...
        if ($("#txtFTPServerIP").val() == "") {
            $("#divEditValidations")[0].innerHTML += "<li class='message'>SFTPServerIP</li>";
            isSuccess = false;
        }
        if ($("#txtFTPFilePath").val() == "") {
            $("#divEditValidations")[0].innerHTML += "<li class='message'>SFTPFilePath</li>";
            isSuccess = false;
        }
        if ($("#txtFTPUsername").val() == "") {
            $("#divEditValidations")[0].innerHTML += "<li class='message'>SFTPUsername</li>";
            isSuccess = false;
        }
        if ($("#txtFTPPassword").val() == "" && $("#hdnReportScheduleId").val() == 0) {
            $("#divEditValidations")[0].innerHTML += "<li class='message'>SFTPPassword</li>";
            isSuccess = false;
        }

    } else if ($("#ComboTransferType").val() == 2) {
        if ($("#txtFTPServerIP").val() == "") {
