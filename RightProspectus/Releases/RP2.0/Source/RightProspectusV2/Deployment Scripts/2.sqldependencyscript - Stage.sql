...
IF NOT EXISTS(SELECT principal_id FROM sys.database_principals WHERE name = 'ECOMAD\stage-pool-rpconnect') 
BEGIN
 CREATE USER [ECOMAD\stage-pool-rpconnect] FOR LOGIN [ECOMAD\stage-pool-rpconnect] WITH DEFAULT_SCHEMA = [stage-pool-rpconnect]
END

IF NOT EXISTS (SELECT  schema_name FROM   information_schema.schemata WHERE   schema_name = 'stage-pool-rpconnect' ) 
BEGIN
  EXEC sp_executesql N'CREATE SCHEMA [stage-pool-rpconnect] AUTHORIZATION [ECOMAD\stage-pool-rpconnect]' 
END
...
GRANT SELECT to rpv2sql_dependency 
    ...
    EXEC sp_addrolemember 'rpv2sql_dependency', [ECOMAD\stage-pool-rpconnect]


	EXEC sp_addrolemember 'db_datareader', [ECOMAD\stage-pool-rpconnect];
	
	EXEC sp_addrolemember 'db_datawriter', [ECOMAD\stage-pool-rpconnect]; 
	
	GRANT EXECUTE ON SCHEMA::dbo TO [ECOMAD\stage-pool-rpconnect];
