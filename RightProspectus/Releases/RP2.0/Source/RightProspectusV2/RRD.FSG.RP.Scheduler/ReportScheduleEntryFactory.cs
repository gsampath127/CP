...
namespace RRD.FSG.RP.Scheduler
{
        ...
        :ReportScheduleEntryFactoryBase<ReportScheduleEntry>
    {
        ...
        {
            return CreateScheduleEntry((int)record["ReportScheduleId"], record["ReportName"].ToString(), (int)record["ClientId"], record["ClientName"].ToString(), (int)record["Status"], (int)record["FrequencyType"], (int)record["FrequencyInterval"],record.GetDateTime(record.GetOrdinal("UtcNextScheduledRunDate")), record["ServiceName"].ToString(), (int)record["ExecutionCount"], record["Email"].ToString(), record["FTPServerIP"].ToString(), record["FTPFilePath"].ToString(), record["FTPUsername"].ToString(), record["FTPPassword"].ToString());
        }
        ...
        {
            return new ReportScheduleEntry(reportScheduleId, reportName, clientId, clientName, status, frequencyType, frequencyInterval ,utcNextScheduledRunDate, serviceName, executionCount, email, ftpServerIP, ftpFilePath, ftpUserName, ftpPassword);
        }
